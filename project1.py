...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    if label*(current_theta_0 + np.matmul(current_theta,feature_vector)) <= 0:
...
def classify(feature_matrix, theta, theta_0):
    ...
    predictions = np.where(predictions >= np.finfo(float).eps, 1.0, -1.0)
    return predictions
...
def classifier_accuracy(
    ...
    validation_accuracy = accuracy(val_prediction, val_labels)
...
def extract_bow_feature_vectors(reviews, dictionary):
    ...
    for i, text in enumerate(reviews):
        word_list = extract_words(text)
        words = {i:word_list.count(i) for i in set(word_list)}
        for word in word_list:
            if word in dictionary:
                feature_matrix[i, dictionary[word]] = words[word]
